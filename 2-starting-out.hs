...
splitAtIndex k l = splitAtIndex' k l []
splitAtIndex' 0 l acc = (reverse(acc), l)
splitAtIndex' k (l:ls) acc = splitAtIndex' (k-1) ls (l:acc)

-- Drop the element at index k in list l
-- For example "dropK 3 [0,0,0,1,0,0,0]" returns [0,0,0,0,0,0]
dropK k l =  (fst split) ++ (tail (snd split))
    where split = splitAt k l 
...
insertElem x k l = fst(split) ++ [x] ++ snd(split)
    where split = splitAt k l

-- Rotate list l n places left.
-- For example, "rotate 2 [1,2,3,4,5]" gives [3,4,5,1,2]
rotate n l = snd(split) ++ fst(split)
	where split = splitAt n l
